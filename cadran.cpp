 ...
 */
Cadran::Cadran(const int _posX, const int _posY, const int _hauteur, const int _largeur,const int _couleurTexte,const int _backgroundColor):
    posX(_posX+1),
    posY(_posY+1),
    hauteur(_hauteur),
    largeur(_largeur),
...
void Cadran::Afficher(const int _valeur, const int _position)
{
   ...
   // cout << "\e[0;33;42m";
     cout << "\e[0;"<<couleurTexte<<";"<<backgroundColor<<"m";
    cout << setfill('0') << setw(2) << _valeur ;
    cout << "\e[0m" << flush;            // Remet à zéro les attributs de couleur
    PositionnerCurseur(posX+1,posY + _position);
}

void Cadran::Effacer()
{
    PositionnerCurseur(hauteur,largeur);
